package isdcm.models;

import isdcm.tools.Database;
...
public class Video {
    ...
    private static final String QUERY_UPDATE = "UPDATE Videos SET plays=plays+1 WHERE id = ? AND user_id = ?";
    private static final String QUERY_VIDEO_ID = "SELECT * FROM Videos WHERE id = ?";
    ...
    public static boolean updateReproductions(int id_video, int id_user) throws SQLException {
        PreparedStatement statement = Database.instance().connection.prepareStatement(QUERY_UPDATE);
    ...
    public static Video getVideoById(int video_id) throws SQLException {
        PreparedStatement statement = Database.instance().connection.prepareStatement(QUERY_VIDEO_ID);
        ...
        if (result.next()) {
            ...
            oRes = new Video(id, title, author, creation_date, duration, plays, description, user_id, format, path);
        } 
